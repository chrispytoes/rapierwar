...
if RapierWarGameMode == nil then
  Log('creating rapierwar game mode' )
  RapierWarGameMode = {}
  RapierWarGameMode.szEntityClassName = "rapierwar"
  RapierWarGameMode.szNativeClassName = "dota_base_game_mode"
  RapierWarGameMode.__index = RapierWarGameMode
end

function RapierWarGameMode:new( o )
  Log('RapierWarGameMode:new' )
  o = o or {}
  setmetatable( o, RapierWarGameMode )
  return o
end

function RapierWarGameMode:InitGameMode()
  ...
  InitLogFile( "log/rapierwar.txt","")

  -- Hooks
  ListenToGameEvent('entity_killed', Dynamic_Wrap(RapierWarGameMode, 'OnEntityKilled'), self)
  ListenToGameEvent('player_connect_full', Dynamic_Wrap(RapierWarGameMode, 'AutoAssignPlayer'), self)
  ListenToGameEvent('player_disconnect', Dynamic_Wrap(RapierWarGameMode, 'CleanupPlayer'), self)
  ListenToGameEvent('dota_item_purchased', Dynamic_Wrap(RapierWarGameMode, 'ShopReplacement'), self)
  ListenToGameEvent('player_say', Dynamic_Wrap(RapierWarGameMode, 'PlayerSay'), self)
  ListenToGameEvent('player_connect', Dynamic_Wrap(RapierWarGameMode, 'PlayerConnect'), self)
  --ListenToGameEvent('player_info', Dynamic_Wrap(RapierWarGameMode, 'PlayerInfo'), self)
  ListenToGameEvent('dota_player_used_ability', Dynamic_Wrap(RapierWarGameMode, 'AbilityUsed'), self)

  Convars:RegisterCommand( "command_example", Dynamic_Wrap(RapierWarGameMode, 'ExampleConsoleCommand'), "A console command example", 0 )
  ...
  Convars:RegisterCommand('fake', function()
    ...
    if not Convars:GetCommandClient() or DEBUG then
      ...
      self:CreateTimer('assign_fakes', {
        endTime = Time(),
        callback = function(rapierwar, args)
          for i=0, 9 do
...
function RapierWarGameMode:CaptureGameMode()
  if GameMode == nil then
    ...
    Log('Beginning Think' ) 
    GameMode:SetContextThink("BarebonesThink", Dynamic_Wrap( RapierWarGameMode, 'Think' ), 0.1 )
  end 
end

function RapierWarGameMode:AbilityUsed(keys)
  Log('AbilityUsed')
  PrintTable(keys)
end

-- Cleanup a player when they leave
function RapierWarGameMode:CleanupPlayer(keys)
  Log('Player Disconnected ' .. tostring(keys.userid))
end

function RapierWarGameMode:CloseServer()
  -- Just exit
  SendToServerConsole('exit')
end

function RapierWarGameMode:PlayerConnect(keys)
  Log('PlayerConnect')
  PrintTable(keys)
...
function RapierWarGameMode:PlayerSay(keys)
  Log('PlayerSay')
  PrintTable(keys)
...
function RapierWarGameMode:AutoAssignPlayer(keys)
  Log('AutoAssignPlayer')
  PrintTable(keys)
  RapierWarGameMode:CaptureGameMode()
...
function RapierWarGameMode:LoopOverPlayers(callback)
...
function RapierWarGameMode:ShopReplacement( keys )
  Log('ShopReplacement' )
  PrintTable(keys)
...
function RapierWarGameMode:getItemByName( hero, name )
  -- Find item by slot
  for i=0,11 do
...
function RapierWarGameMode:Think()
  ...
  if RapierWarGameMode.t0 == nil then
    RapierWarGameMode.t0 = now
  end
  local dt = now - RapierWarGameMode.t0
  RapierWarGameMode.t0 = now

  --RapierWarGameMode:thinkState( dt )

  -- Process timers
  for k,v in pairs(RapierWarGameMode.timers) do
    ...
    if (bUseGameTime and GameRules:GetGameTime() > v.endTime) or (not bUseGameTime and Time() > v.endTime) then
      ...
      RapierWarGameMode.timers[k] = nil

      -- Run the callback
      local status, nextCall = pcall(v.callback, RapierWarGameMode, v)
      ...
      if status then
        ...
        if nextCall then
          ...
          v.endTime = nextCall
          RapierWarGameMode.timers[k] = v
        end

      else
        -- Nope, handle the error
        RapierWarGameMode:HandleEventError('Timer', k, nextCall)
      end
    end
  end
...
function RapierWarGameMode:HandleEventError(name, event, err)
...
function RapierWarGameMode:CreateTimer(name, args)
  --[[
  args: {
...
function RapierWarGameMode:RemoveTimer(name)
  -- Remove this timer
  self.timers[name] = nil
end

function RapierWarGameMode:RemoveTimers(killAll)
  local timers = {}
...
function RapierWarGameMode:ExampleConsoleCommand()
...
function RapierWarGameMode:OnEntityKilled( keys )
...
function dealDamage(source, target, damage)
  ...
  RapierWarGameMode:CreateTimer(DoUniqueString("damage"), {
